...
if [ "$(whoami)" != "${EFFECTIVE_TARGET_USER}" ]; then
	...
	FULL_COMMAND_LINE="${0} ${@}"
        exec sudo -u ${EFFECTIVE_TARGET_USER} --preserve-env=ARKSERVER_MAP,ARKSERVER_PORT,ARKSERVER_QUERYPORT,ARKSERVER_RCONENABLED,ARKSERVER_RCONPASSWORD,ARKSERVER_RCONPORT,ARKSERVER_MAXPLAYERS,ARKSERVER_ADDITIONALOPTIONS,ARKSERVER_ADDITIONALARGUMENTS -- sh -c "START_TIME=\"${START_TIME}\" LOG_FILE=\"${LOG_FILE}\" exec ${FULL_COMMAND_LINE}"
        exit $?
fi
